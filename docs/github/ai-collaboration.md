...
The exact pre-activation closed Issues listed in `docs/github/ai-lifecycle-historical-exemptions.json` remain read-only historical records. The registry is bound to the activation commit and each entry's original `closed_at` plus expected legacy lifecycle status: Issues #1–#7 retain their pre-activation `status:done` label, while #35, #37, #41, #46, and #49 have no lifecycle label. Scheduled reconciliation skips an entry only while it is closed as completed with that exact timestamp and status shape, has exactly one managed type, and has no workflow-authored lifecycle receipt. A missing entry, snapshot mismatch, prior receipt, or malformed type/state returns the Issue to the normal fail-closed path. Outside this exact registry, every `status:done` Issue must re-prove its final terminal receipt, claim-bound unique merged pull request, closure chronology, and merge reachability from the default branch. An older closed-unmerged PR is accepted only when its exact claim generation records `pr-open`, `pr-close`, and a later `release` or `expire`; arbitrary closed PRs are not ignored. The controller never synthesizes claims, lifecycle labels, or terminal receipts for historical records.
