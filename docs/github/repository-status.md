...
- Repository-wide reconciliation audit: manual workflow-dispatch run [33868540482](https://github.com/EthanSMC/qwen-harness-bridge/actions/runs/33868540482) preserved the initial red observation for five closed pre-enrollment auxiliary Issues. A stricter terminal-state review also identified seven completed implementation Issues (#1–#7) that were labeled `status:done` by pre-activation synchronization and have no workflow lifecycle receipts. Issue [#56](https://github.com/EthanSMC/qwen-harness-bridge/issues/56) tracks the bounded repair for all 12 immutable snapshots. The exception registry is activation-bound and records exact Issue number, original `closed_at`, and expected legacy status; reconciliation additionally requires closed state, exactly one managed type, and no workflow receipt. Every mismatch follows normal strict validation, including full terminal-evidence reconstruction for non-exempt `status:done` Issues.
