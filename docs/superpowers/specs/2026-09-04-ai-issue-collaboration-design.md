...
Existing closed Issues do not require synthetic claim receipts. Repository-wide reconciliation reports and skips a closed pre-lifecycle Issue that has a managed type but zero lifecycle labels; it still fails closed if that Issue has any malformed or inconsistent lifecycle state. Synchronization accepts `status:done` only after finding exactly one merged closing pull request, valid close-after-merge chronology, and a merge commit reachable from `main`. Existing open pull requests receive a documented one-time migration receipt or must re-enter the lifecycle from `status:ready`; migrations are honored only while activation is null and are never accepted afterward, even in report mode.
