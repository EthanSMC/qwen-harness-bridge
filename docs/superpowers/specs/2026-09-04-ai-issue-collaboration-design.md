...
Existing closed Issues do not require synthetic claim receipts. Repository-wide reconciliation reports and skips only an exact entry in the activation-bound historical-exemption registry whose Issue number and `closed_at` snapshot still match, whose current shape is closed with exactly one managed type and zero lifecycle labels, and whose complete comment history contains no workflow lifecycle receipt. Any missing entry, changed close timestamp, prior receipt, or malformed type/state is processed normally and fails closed. Synchronization accepts `status:done` only after finding exactly one merged closing pull request, valid close-after-merge chronology, and a merge commit reachable from `main`. Existing open pull requests receive a documented one-time migration receipt or must re-enter the lifecycle from `status:ready`; migrations are honored only while activation is null and are never accepted afterward, even in report mode.
