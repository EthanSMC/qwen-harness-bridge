...
The pull-request body validator only verifies the selected evidence mode and the supplied GitHub Actions run or PR checks URL. Before merging, the controller must independently query the GitHub checks API and confirm all required checks succeeded; the body evidence URL is not itself a CI result.
